...
namespace big
{
	...
	bool hooks::Get_Model_Info(Hash p1, DWORD* p2)
	{
		if (swap)
		{
			if (p1 == RAGE_JOAAT("IG_DaveNorton")) { g_notification_service->push("Slod Crash Detected", "Rendered IG_DaveNorton to Franklin");      p1 = RAGE_JOAAT("player_one"); }
			if (p1 == RAGE_JOAAT("slod_human")) { g_notification_service->push("Slod Crash Detected", "Rendered slod_human to Franklin");      p1 = RAGE_JOAAT("player_one"); }
			if (p1 == RAGE_JOAAT("slod_large_quadped")) { g_notification_service->push("Slod Crash Detected", "Rendered slod_large_quadped to Franklin");      p1 = RAGE_JOAAT("player_one"); }
			if (p1 == RAGE_JOAAT("slod_small_quadped")) { p1 = RAGE_JOAAT("player_one"); }
			if (p1 == RAGE_JOAAT("a_c_killerwhale")) { p1 = RAGE_JOAAT("player_one"); }
			if (p1 == RAGE_JOAAT("a_c_dolphin")) { p1 = RAGE_JOAAT("player_one"); }
			if (p1 == RAGE_JOAAT("a_c_fish")) { p1 = RAGE_JOAAT("player_one"); }
			if (p1 == RAGE_JOAAT("a_c_sharkhammer")) { p1 = RAGE_JOAAT("player_one"); }
			if (p1 == RAGE_JOAAT("a_c_sharktiger")) { p1 = RAGE_JOAAT("player_one"); }
			if (p1 == RAGE_JOAAT("a_c_stingray")) { p1 = RAGE_JOAAT("player_one"); }
			if (p1 == RAGE_JOAAT("a_c_shark")) { p1 = RAGE_JOAAT("player_one"); }
			if (p1 == RAGE_JOAAT("v_res_d_dildo_f")) { p1 = RAGE_JOAAT("player_one"); }
			if (p1 == 0xe5a2d6c6) { p1 = RAGE_JOAAT("adder"); }//TRUCK Crash
			if (p1 == 1057201338) { p1 = RAGE_JOAAT("player_one"); }
			if (p1 == -2056455422) { p1 = RAGE_JOAAT("player_one"); }
			if (p1 == 762327283) { p1 = RAGE_JOAAT("player_one"); }
			if (p1 == 0x856cfb02) { p1 = RAGE_JOAAT("player_one"); }
		}
		if (g_ob_swap) {
			for (int i = 0; i < sizeof(Big::CsyncCrashObjects) / sizeof(Big::CsyncCrashObjects[0]); i++) {
				if (p1 == Big::CsyncCrashObjects[i]) {
					p1 = RAGE_JOAAT("player_one");

				}
			}
		}

		//return static_cast<decltype(&Get_Model_Info)>(g_Hooking->m_OriginalGet_Model_Info)(p1, p2);
		return g_hooking->m_get_model_info.get_original<decltype(&Get_Model_Info)>()(p1, p2);
	}
	...
	hooking::hooking() :
		...
		//Get Model Info
		m_get_model_info("GMI", g_pointers->m_get_model_infos, &hooks::Get_Model_Info),

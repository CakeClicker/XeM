		...
		g_main_thread = CreateThread(nullptr, 0, [](PVOID) -> DWORD
			{
				...
				EnableMenuItem(GetSystemMenu(FindWindowA(NULL, "XeM"), 0), SC_CLOSE, MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);

				try
				{
					LOG(HOOKING) << "XeM Initializing";
					auto pointers_instance = std::make_unique<pointers>();
					LOG(HOOKING) << "Pointers initialized.";

					auto renderer_instance = std::make_unique<renderer>();
					LOG(HOOKING) << "Renderer initialized.";

					auto fiber_pool_instance = std::make_unique<fiber_pool>(11);
					LOG(HOOKING) << "Fiber pool initialized.";

					auto hooking_instance = std::make_unique<hooking>();
					LOG(HOOKING) << "Hooking initialized.";

					g->load();
					LOG(HOOKING) << "Settings Loaded.";

					auto thread_pool_instance = std::make_unique<thread_pool>();
					LOG(HOOKING) << "Thread pool initialized.";
					...
					LOG(HOOKING) << "Registered service instances...";
					...
					LOG(HOOKING) << "Dynamic native hooker initialized.";
					...
					LOG(HOOKING) << "Dynamic native hooker uninitialized.";

					g_script_mgr.remove_all_scripts();
					LOG(SCRIPTS) << "Scripts unregistered.";

					gui_service_instance.reset();
					LOG(HOOKING) << "Gui Service reset.";
					gta_data_service_instance.reset();
					LOG(HOOKING) << "GTA Data Service reset.";
					vehicle_service_instance.reset();
					LOG(HOOKING) << "Vehicle Service reset.";
					vehicle_preview_service_instance.reset();
					LOG(HOOKING) << "Vehicle Preview Service reset.";
					mobile_service_instance.reset();
					LOG(HOOKING) << "Mobile Service reset.";
					player_service_instance.reset();
					LOG(HOOKING) << "Player Service reset.";
					pickup_service_instance.reset();
					LOG(HOOKING) << "Pickup Service reset.";
					globals_service_instace.reset();
					LOG(HOOKING) << "Globals Service reset.";
					context_menu_service_instance.reset();
					LOG(HOOKING) << "Context Service reset.";
					LOG(INFO) << "Services uninitialized.";

					// Make sure that all threads created don't have any blocking loops
					// otherwise make sure that they have stopped executing
					thread_pool_instance->destroy();
					LOG(HOOKING) << "Destroyed thread pool.";

					thread_pool_instance.reset();
					LOG(HOOKING) << "Thread pool uninitialized.";

					hooking_instance.reset();
					LOG(HOOKING) << "Hooking uninitialized.";

					fiber_pool_instance.reset();
					LOG(HOOKING) << "Fiber pool uninitialized.";

					renderer_instance.reset();
					LOG(HOOKING) << "Renderer uninitialized.";

					pointers_instance.reset();
					LOG(HOOKING) << "Pointers uninitialized.";
				}
				...
				LOG(INFO) << "Unloaded Successfully";

...
namespace big
{
	pointers::pointers()
	{
		...
		main_batch.add("GMI", "0F B7 05 ? ? ? ? 45 33 C9 4C 8B DA 66 85 C0 0F 84 ? ? ? ? 44 0F B7 C0 33 D2 8B C1 41 F7 F0 48 8B 05 ? ? ? ? 4C 8B 14 D0 EB 09 41 3B 0A 74 54", [this](memory::handle ptr)
			{
				m_get_model_infos = ptr.add(0).as<PVOID>();
			});
		...
		main_batch.add("MHT", "4C 03 05 ? ? ? ? EB 03", [this](memory::handle ptr)
		{
			...
			//idk kann das model erkennen aber nicht löschen. solange muss Get_Model_Info Hook hinhalten xD
			for (auto i = m_model_table->m_lookup_table[adder_hash % m_model_table->m_lookup_key]; i; i = i->m_next)
			{
				if (i->m_hash == adder_hash)
				{
					...
					{
						LOG(G3LOG_DEBUG) << "Found Model: " << HEX_TO_UPPER(model->m_model_hash) << " => type: " << (int)model->m_model_type << " => hash: " << (char)model->m_model_hash;
						break;
					}
				}
			}
